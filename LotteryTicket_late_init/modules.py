...
class CNN(nn.Module):
    ...
    def prune(self, rat): 
        ...
        for layer in self.model:
            if isinstance(layer, nn.Conv2d):
                print("prune weights in layer [{}] about [{}][%]".format(curr,rat))
                """
                ...
                weight = abs(weight)
                weight[weight==0] = max(weight)+1
                indicies = torch.argsort(weight)[:prune_params] #for array
